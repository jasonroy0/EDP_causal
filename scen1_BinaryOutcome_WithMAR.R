...
# Code for BNP generative model for simulation 1 (binary outcome with
# missing data in some covariates) from the following paper on ArXiv:
...
file_name <- paste("Results_testErrorCatch/bnp_edp_scen1_binaryOutcome_WithMAR_n1000_ns1000run_ID", 
  run_ID, ".rds", sep = "")
...
L_mar[, 1] <- rbinom(n, 1, expit(-1.5 + L[, 2] + Y))
L_mar[, 2] <- rbinom(n, 1, expit(-1.5 + L[, 3] + A))
L_mar[, 3] <- rbinom(n, 1, expit(-1 - A + Y))
L_mar[, 4] <- rbinom(n, 1, expit(-0.4 - L[, 1] - L[, 2]))
