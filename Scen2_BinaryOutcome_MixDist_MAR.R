# Jason Roy, Kirsten J. Lum, Mike J. Daniels, Bret Zeldow, Jordan Dworkin,
# and Vincent Lo Re III

# February 2017

# Code for BNP generative model for simulation 2 (binary outcome with
# mixture distribution and missing data in some covariates) 
# from the following paper on ArXiv: https://arxiv.org/list/stat/

# Roy J, Lum KJ, Daniels MJ, Zeldow B, Dworkin J, and Lo Re III V.
# Bayesian Nonparametric Generative Models for Causal Inference with
# Missing at Random Covariates.(submitted for review in Feb 2017)

# IMPORTANT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!:
# Make a folder called 'Results' in the folder where this R code is run.
# This code is written to run the simulation on a LSF computing cluster.
# Each simulated dataset is processed on a separate node. For each
# simulated dataset, the analysis results are written to an .rds file. The
# 'Results' folder is used to store the .rds files. Use the R code,
# 'GatherResults.R' to read in the .rds files and calculate bias, coverage
# and empirical standard deviation (ESD).
...
# Load Cpp code.  This code must be in your current directory or you can
# change the path.
...
# IMPORTANT: Make file_name where results will be stored for each dataset.  
# Make a folder called 'Results' in the folder where this code is run.
file_name <- paste("Results/bnp_edp_scen2_binaryOutcome_MixDist_MAR_n1000_ns1000run_ID", 
  run_ID, ".rds", sep = "")
...
# Define number of MCMC draws to 'burn' in Gibbs Sampler (check
# convergence)
# for small n (e.g. n <- 250) burn_in<-10000
...
# Define number of observations per draw when using Monte Carlo integration
# to integrate out confounders
M = 1000
...
# Random number function and for the (scaled) inverse-chi-squared
# distribution.
...
update_beta_coeff <- function(beta_coeff_current, x_temp, y_temp) {
  ...
  loglike_prop <- sum(dbinom(y_temp, 1, prob = expit(x_temp %*% proposed), 
    log = TRUE)) + dmvn(proposed, beta_coeff_init, (diagbetacov0), log = TRUE) + 
    dmvn(beta_coeff_current, proposed, diag(c(rep(0.01, p))), log = TRUE)
  
  loglike_old <- sum(dbinom(y_temp, 1, prob = expit(x_temp %*% beta_coeff_current), 
...
generate_data <- function(n) {
  ...
  proba<-expit(1.3-.8*L[,1])
  A<-rbinom(n,1,proba)
  
  # Generate outcome Y
   part1prob<-2*exp(-2*(L[,1]-4)^2)/(2*exp(-2*(L[,1]-4)^2)+2*exp(-2*(L[,1]-6)^2))
  part1<-rbinom(n,1,part1prob)
  
  proby1<-part1*expit(-.8-0.1*L[,1]+1*A)+(1-part1)*expit(-2+0.45*L[,1])
  Y <- rbinom(n, 1,   proby1)
...
L_mar[, 1] <- rbinom(n,1,expit(-2+A+Y))
...
for (i in 1:100000) {
  ...
  h0ya1 <- h0ya1 + expit(cbind(1, 1, X[, ((ptx + 1):(ptx + p1 + p2))]) %*% 
    beta_coeff_prior)
  h0ya0 <- h0ya0 + expit(cbind(1, 0, X[, ((ptx + 1):(ptx + p1 + p2))]) %*% 
    beta_coeff_prior)
...
for (gibbsreps in 1:gibbs_total) {
  ...
  for (j in 1:k) {
    ...
    beta_coeff[j, ] <- update_beta_coeff(beta_coeff_current, X_matrix[s[, 1] == j, ], 
	Y[s[, 1] == j])
  }
  ...
  for (r in 1:p1) {
    ...
    tempprobs <- cbind((1 - x_prob_param[rowx[L_mar[, r] == 1], (r + ptx)]) * 
      expit(rowSums(tempx0 * (beta_coeff[s[L_mar[, r] == 1, 1], ]))), 
      x_prob_param[rowx[L_mar[, r] == 1], (r + ptx)] * expit(rowSums(tempx1 * 
  ...
  for (r in (p1 + 1):(p1 + p2)) {
    ...
    nmiss <- length(proposed)
    propL <- matrix(nrow = nmiss, ncol = p2, 0)
    propL[, r] <- proposed
    propX_matrix <- X_matrix[L_mar[, r] == 1, ]
    propX_matrix[, (r + 1 + ptx)] <- proposed
    
    loglikeprop <- dnorm(propL[, r], x_mean_param[rowx[L_mar[, r] == 1], 
      (r - p1)], sqrt(x_var_param[rowx[L_mar[, r] == 1], (r - p1)]), 
      log = TRUE) + dbinom(Y[L_mar[, r] == 1], 1, expit(rowSums(propX_matrix * 
    ...
    loglikeold <- dnorm(L[L_mar[, r] == 1, r], x_mean_param[rowx[L_mar[, 
      ...
      p1)]), log = TRUE) + dbinom(Y[L_mar[, r] == 1], 1, expit(rowSums(X_matrix[L_mar[, 
  ...
  for (i in 1:MCnum) {
    beta_coeff_prior <- as.numeric(rmvn(1, beta_coeff_init, (diagbetacov0)))
    h0ya1 <- h0ya1 + expit(cbind(1, 1, X[, (2:(ptx + p1 + p2))]) %*% beta_coeff_prior)
    h0ya0 <- h0ya0 + expit(cbind(1, 0, X[, (2:(ptx + p1 + p2))]) %*% beta_coeff_prior)
    h0y <- h0y + dbinom(Y, 1, expit(X_matrix %*% beta_coeff_prior))
  }
  h0ya1 <- h0ya1/MCnum
  h0ya0 <- h0ya0/MCnum
  ...
  if (gibbsreps >= burn_in & gibbsreps%%100 == 0) 
    {
      ...
      # h0xa1.mc[i,1:ptx] and h0xa0.mc[i,1:ptx] were calculated before Gibbs sampling loop
